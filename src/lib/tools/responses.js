const { EmbedBuilder, ButtonBuilder, GuildMember, Guild } = require("discord.js");
...
class ScrimsMessageBuilder {
    ...
    static parseUser(resolvable, scrimsUsers, guild) {
        ...
        let matches = scrimsUsers.filter(user => user.user_id === resolvable || user.discord_id === resolvable || user.tag === resolvable)
    ...
    /** @param {number} delta Number of seconds to put into words */
    static stringifyTimeDelta(delta) {
    ...
    static cancelButton(i18n=I18n.getInstance()) {
        ...
        return new ButtonBuilder().setLabel(i18n.get('cancel')).setStyle(2).setCustomId(`CANCEL`); 
     ...
     * @template T
     * @param { T[] } items 
     * @param { (items: T[], index: number, containers: T[][]) => EmbedBuilder } getEmbed
     */
