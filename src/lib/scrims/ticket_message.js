...
class ScrimsTicketMessage extends TableRow {
    ...
    static fromMessage(message) {
        return new ScrimsTicketMessage(message.client.database)
            .setMessage(message)
            .setContent(message.content || message.embeds[0]?.title || '*No Content*')
            .setReferenceId(message?.reference?.messageId ?? null)
            .setAuthor(message.author)
            .setCreatedPoint(message.createdTimestamp)
    }
    ...
    get author() {
        ...
        return this.bot.users.resolve(this.author_id) ?? null;
    }
